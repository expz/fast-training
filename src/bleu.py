#coding: UTF-8
"""
This file is taken from the BLEU4Python repository at
        ...
        for i in range(n_ref):
            ...
            for n in range(max_n):
                ...
                for ngram in sent_ngram.keys():
                    if ngram not in ref_ngram or ref_ngram[ngram] < sent_ngram[
                            ngram]:
...
def corpus_bleu(hypothesis, references, max_n=4):
    ...
    if len(hypothesis) == 0:
        return [0.0] * 5, [0.0] * 4
    clip_count, count, total_len_hyp, total_len_ref = bleu_count(hypothesis,
                                                                 references,
                                                                 max_n=max_n)
    brevity_penalty = 1.0
    bleu_scores = []
        ...
        for i in range(x_len):
            ...
            for n in range(max_n):
                ...
                if st >= 0:
                    ...
                    hyp_ngram[ngram] += 1
                    ret_count[-1][-1][n] += 1
                    if ngram in ref_ngram and hyp_ngram[ngram] <= ref_ngram[
                            ngram]:
...
def incremental_sent_bleu(hypothesis, references, max_n=4):
    clip_count, count, total_len_hyp, total_len_ref = incremental_bleu_count(
        ...
        if total_len_hyp[i] < total_len_ref[i]:
            if total_len_hyp[i] == 0:
                ...
                brevity_penalty = math.exp(1 -
                                           total_len_ref[i] / total_len_hyp[i])
...
def incremental_test_corpus_bleu(hypothesis, references, max_n=4):
    assert (len(hypothesis) == len(references))
    tmp_clip_count, tmp_count, tmp_total_len_hyp, tmp_total_len_ref = incremental_bleu_count(
        hypothesis, references, max_n=max_n)
    clip_count = [0] * 4
    count = [0] * 4
    total_len_hyp = 0
